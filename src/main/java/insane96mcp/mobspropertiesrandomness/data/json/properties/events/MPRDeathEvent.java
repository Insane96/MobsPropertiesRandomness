...
public class MPRDeathEvent extends MPREvent {
	...
	public MPRDeathEvent(int updateInterval, ResourceLocation id, Target target, @Nullable CommandFunction.CacheableFunction function, @Nullable List<MPRProperty> applyProperties, List<MPRCondition> conditions) {
	...
	public static void tickEvents(LivingEntity entity) {
		List<MPREvent> events = getEvents(entity, MPRDeathEvent.class);
		for (MPREvent event : events)
			((MPRDeathEvent) event).tick(entity);
	}
	...
	public static class Serializer implements JsonDeserializer<MPRDeathEvent>, JsonSerializer<MPRDeathEvent> {
		...
		public MPRDeathEvent deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
			...
			return new MPRDeathEvent(GsonHelper.getAsInt(jObject, "update_interval", 20), ResourceLocation.parse(id), Target.THIS, function, properties, conditions);
		}
